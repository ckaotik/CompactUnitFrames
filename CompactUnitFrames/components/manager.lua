...
function addon:SetupManager(manager)
	...
	addon:Manager_DisableCUF(addon.db.frames.disableCUF)

	addon:Manager_SetAlpha(addon.db.frames.pullout.passiveAlpha)
	hooksecurefunc("CompactRaidFrameManager_Expand", function(self)
		addon:Manager_SetAlpha(addon.db.frames.pullout.activeAlpha)
	end)
	hooksecurefunc("CompactRaidFrameManager_Collapse", function(self)
		addon:Manager_SetAlpha(addon.db.frames.pullout.passiveAlpha)
	end)
	addon:MinifyPullout(addon.db.frames.pullout.minify)
	--]]
...
function addon:MinifyPullout(enable)
	...
	if enable then
		...
		--[[hooksecurefunc("CompactRaidFrameManager_Expand", function(self)
			self:SetAlpha(1)
			self:SetPoint("TOPLEFT", UIParent, "TOPLEFT", (addon.db.frames.pullout.posX or 0) -7, addon.db.frames.pullout.posY or -140)
		...
		hooksecurefunc("CompactRaidFrameManager_Collapse", function(self)
			self:SetAlpha(addon.db.frames.pullout.alpha or 1)
			self:SetPoint("TOPLEFT", UIParent, "TOPLEFT", addon.db.frames.pullout.posX or 0, addon.db.frames.pullout.posY or -140)
			SetCRFManagerSize(self)
