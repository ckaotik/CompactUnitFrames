local MAJOR, MINOR = 'LibOptionsGenerate-1.0', 1
...
local optionsTable = {
	type = 'group',
	args = {
		['MidgetLocalDB.foo.bar'] = { -- keyed by variableName that contains data
			type = 'group',
			inline = true,
		...
		global = { -- or using .descStyle (which we are abusing here since AceConfigRegistry won't validate extra fields)
			type = 'group',
			inline = true,
			name = 'Shared Settings',
			descStyle = 'MidgetDB.baz',
			order = 2,
			args = {},
		},
	},
}
optionsTable = LibStub('LibOptionsGenerate-1.0'):GenerateOptions(optionsTable)
--]]
local emptyTable = {}
